package Bio::EnsEMBL::GlyphSet::ciona_dbest_ncbi_transcript;
use strict;
use vars qw(@ISA);
...
sub my_label {
    return "3/5' EST genes (dbEST)";
}

sub colours {
    my $self = shift;
    my $Config = $self->{'config'}->get('ciona_dbest_ncbi_transcript','colours');
}
...
sub href {
  ...
  return ( $self->{'config'}->get('ciona_dbest_ncbi_transcript','_href_only') eq '#tid' && exists $highlights{lc($gene->stable_id())} ) ?
        "#$tid" : 
...
sub gene_href {
  ...
  my $gid = $gene->stable_id();
  return ( $self->{'config'}->get('ciona_dbest_ncbi_transcript','_href_only') eq '#gid' && exists $highlights{lc($gid)} ) ?
...
sub features {
  my ($self) = @_;
  return $self->{'container'}->get_all_Genes( 'ciona_dbest_ncbi' );
}

sub legend {
  my ($self, $colours) = @_;
  return ('ciona_genes', 1000, 
    [ "3/5' EST genes (dbEST)" => $colours->{'dbest_ncbi'}[0], ]
  );
}

sub error_track_name { return "3/5' EST genes (dbEST)"; }
