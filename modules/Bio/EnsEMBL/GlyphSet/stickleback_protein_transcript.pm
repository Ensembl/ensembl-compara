package Bio::EnsEMBL::GlyphSet::stickleback_protein_transcript;
use strict;
use vars qw(@ISA);
...
sub my_label {
    return 'Stickleback trans.';
}

sub colours {
    my $self = shift;
    my $Config = $self->{'config'}->get('stickleback_protein_transcript','colours');
}
...
sub colour {
  ...
  return (@{$colours->{'stickleback_protein'}}, $highlight);
}
...
sub href {
  ...
  return ( $self->{'config'}->get('stickleback_protein_transcript','_href_only') eq '#tid' && exists $highlights{lc($gene->stable_id())} ) ?
        "#$tid" : 
...
sub gene_href {
  ...
  my $gid = $gene->stable_id();
  return ( $self->{'config'}->get('stickleback_protein_transcript','_href_only') eq '#gid' && exists $highlights{lc($gid)} ) ?
...
sub features {
  my ($self) = @_;
  my $track = 'stickleback_protein_transcript';
  my $slice = $self->{'container'};
  my @genes;
  foreach my $analysis( 'stickleback_protein' ){
...
sub legend {
  my ($self, $colours) = @_;
  return ('stickleback_protein', 1000, 
    [ 'Stickleback genes' => $colours->{'stickleback_protein'}[0], ]
  );
}

sub error_track_name { return 'Stickleback proteins'; }
