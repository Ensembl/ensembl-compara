package Bio::EnsEMBL::GlyphSet::transcriptexon_bgtrack;
use strict;
use vars qw(@ISA);
...
@Bio::EnsEMBL::GlyphSet::transcriptexon_bgtrack::ISA = qw(Bio::EnsEMBL::GlyphSet);
sub _init {
  my ($self) = @_;
 ...
 # my $bitmap_length = $Config->image_width(); #int($Config->container_width() * $pix_per_bp);
  ...
  if( $Config->{'fakeslice'} ) {
    ...
    my $features =  $self->{'container'}->get_all_Transcripts();
    my $stable_id = $Config->{'id'};
    foreach my $transcript ( @$features ) {
      next if $stable_id &&  ($transcript->stable_id() ne $stable_id);
