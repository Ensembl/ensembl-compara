package Bio::EnsEMBL::GlyphSet::cdna_all_transcript;
use strict;
use vars qw(@ISA);
...
sub colours {
    my $self = shift;
    my $Config = $self->{'config'}->get('cdna_all_transcript','colours');
}
...
sub href {
  ...
  return ( $self->{'config'}->get('cdna_all_transcript','_href_only') eq '#tid' && exists $highlights{lc($gene->stable_id())} ) ?
        "#$tid" : 
...
sub gene_href {
  ...
  my $gid = $gene->stable_id();
  return ( $self->{'config'}->get('cdna_all_transcript','_href_only') eq '#gid' && exists $highlights{lc($gid)} ) ?
...
sub features {
  my ($self) = @_;
  return $self->{'container'}->get_all_Genes( 'cdna_all' );
}

sub legend { return }

sub error_track_name { return "cDNA genes"; }
