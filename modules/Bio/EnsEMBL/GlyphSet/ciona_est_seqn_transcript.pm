package Bio::EnsEMBL::GlyphSet::ciona_est_seqn_transcript;
use strict;
use vars qw(@ISA);
...
sub my_label {
    return "5' EST genes (Kyoto)";
}

sub colours {
    my $self = shift;
    my $Config = $self->{'config'}->get('ciona_est_seqn_transcript','colours');
}
...
sub href {
  ...
  return ( $self->{'config'}->get('ciona_est_seqn_transcript','_href_only') eq '#tid' && exists $highlights{lc($gene->stable_id())} ) ?
        "#$tid" : 
...
sub gene_href {
  ...
  my $gid = $gene->stable_id();
  return ( $self->{'config'}->get('ciona_est_seqn_transcript','_href_only') eq '#gid' && exists $highlights{lc($gid)} ) ?
...
sub features {
  my ($self) = @_;
  return $self->{'container'}->get_all_Genes( 'ciona_est_seqn' );
}

sub legend {
  my ($self, $colours) = @_;
  return ('ciona_genes_2', 1002, 
    [ "5' EST genes (Kyoto)" => $colours->{'est_seqn'}[0], ]
  );
}

sub error_track_name { return "5' EST genes (Kyoto)"; }
