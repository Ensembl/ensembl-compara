package Bio::EnsEMBL::GlyphSet::ciona_est_seqs_transcript;
use strict;
use vars qw(@ISA);
...
sub my_label {
    return "full insert cDNA clone";
}

sub colours {
    my $self = shift;
    my $Config = $self->{'config'}->get('ciona_est_seqs_transcript','colours');
}
...
sub href {
  ...
  return ( $self->{'config'}->get('ciona_est_seqs_transcript','_href_only') eq '#tid' && exists $highlights{lc($gene->stable_id())} ) ?
        "#$tid" : 
...
sub gene_href {
  ...
  my $gid = $gene->stable_id();
  return ( $self->{'config'}->get('ciona_est_seqs_transcript','_href_only') eq '#gid' && exists $highlights{lc($gid)} ) ?
...
sub gene_zmenu {
  ...
  my $gid = $gene->stable_id();
  my $zmenu = {
    'caption'                   => "full insert cDNA clone",
    "01:Gene:$gid"          => "/@{[$self->{container}{_config_file_name_}]}/geneview?gene=$gid",
  };
...
sub features {
  my ($self) = @_;
  return $self->{'container'}->get_all_Genes( 'ciona_est_seqs' );
}

sub legend {
  my ($self, $colours) = @_;
  return ('ciona_genes_3', 1003, 
    [ "full insert cDNA clone" => $colours->{'est_seqs'}[0], ]
  );
}

sub error_track_name { return "full insert cDNA clone"; }
