package Bio::EnsEMBL::GlyphSet::vectorbase_transcript;
use strict;
use vars qw(@ISA);
...
sub my_label {
  my $self = shift;
  return $self->{'config'}->{'_draw_single_Transcript'} || $self->{'config'}->{'geneid'} || "Vectorbase trans.";
}

sub colours {
  my $self = shift;
  my $Config = $self->{'config'};
  return $Config->get('vectorbase_transcript','colours');
}

sub features {
  my ($self) = @_;
  my $track = 'vectorbase_transcript';
  my $slice = $self->{'container'};
  my $sp = $self->{'_config_file_name_'};
  my @analyses = ( 'vectorbase', 'pseudogene');
...
sub href {
  ...
  return ( $self->{'config'}->get('vectorbase_transcript','_href_only') eq '#tid' && exists $highlights{lc($gene->stable_id)} ) ?
    "#$tid" : 
...
sub gene_href {
  ...
  my $script_name = $ENV{'ENSEMBL_SCRIPT'} eq 'genesnpview' ? 'genesnpview' : 'geneview';
  return ( $self->{'config'}->get('vectorbase_transcript','_href_only') eq '#gid' && exists $highlights{lc($gene->stable_id)} ) ?
    "#$gid" :
    qq(/@{[$self->{container}{_config_file_name_}]}/$script_name?gene=$gid);
...
sub legend {
  my ($self, $colours) = @_;
  return ('genes', 900, [
    'Vectorbase predicted genes (known)' => $colours->{'_KNOWN'}[0],
    'Vectorbase predicted genes (novel)' => $colours->{'_'}[0],
    'Vectorbase pseudogenes'             => $colours->{'_PSEUDO'}[0],
  ]);
}

sub error_track_name { return 'Vectorbase transcripts'; }
