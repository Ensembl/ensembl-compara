package Bio::EnsEMBL::GlyphSet::ciona_kyotograil_2005_transcript;
use strict;
use vars qw(@ISA);
...
sub my_label {
    return "Kyotograil '05 model";
}

sub colours {
    my $self = shift;
    my $Config = $self->{'config'}->get('ciona_kyotograil_2005_transcript','colours');
}
...
sub colour {
  ...
  my $highlight = undef;
  my $colour = $colours->{lc($gene->biotype())}[0];
  $colour = $colours->{'estgene'}[0] unless $colour;
...
sub href {
  ...
  return ( $self->{'config'}->get('ciona_kyotograil_2005_transcript','_href_only') eq '#tid' && exists $highlights{lc($gene->stable_id())} ) ?
        "#$tid" : 
...
sub gene_href {
  ...
  my $gid = $gene->stable_id();
  return ( $self->{'config'}->get('ciona_kyotograil_2005_transcript','_href_only') eq '#gid' && exists $highlights{lc($gid)} ) ?
...
sub features {
  my ($self) = @_;
  return $self->{'container'}->get_all_Genes( 'ciona_kyotograil_2005' );
}

sub legend {
  my ($self, $colours) = @_;
  return ('ciona_genes_12', 1012, 
    [ "Kyotograil '05 model" => $colours->{'kyotograil_2005'}[0], ]
  );
}

sub error_track_name { return "Kyotograil '05 model"; }
