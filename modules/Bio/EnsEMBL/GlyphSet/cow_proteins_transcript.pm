package Bio::EnsEMBL::GlyphSet::cow_proteins_transcript;
use strict;
use vars qw(@ISA);
...
sub colours {
    my $self = shift;
    my $Config = $self->{'config'}->get('cow_proteins_transcript','colours');
}
...
sub colour {
  ...
  return (@{$colours->{'cow_protein'}}, $highlight); 
}
...
sub href {
  ...
  return ( $self->{'config'}->get('cow_proteins_transcript','_href_only') eq '#tid' && exists $highlights{lc($gene->stable_id())} ) ?
        "#$tid" : 
...
sub gene_href {
  ...
  my $gid = $gene->stable_id();
  return ( $self->{'config'}->get('cow_proteins_transcript','_href_only') eq '#gid' && exists $highlights{lc($gid)} ) ?
...
sub features {
  my ($self) = @_;
  my $track = 'cow_proteins_transcript';
  my $slice = $self->{'container'};
  my @genes;
  foreach my $analysis( 'cow_proteins' ){
...
sub legend {
  my ($self, $colours) = @_;
  return ('cow_proteins', 1000, 
    [ 'Cow genes' => $colours->{'cow_protein'}[0], ]
  );
}
