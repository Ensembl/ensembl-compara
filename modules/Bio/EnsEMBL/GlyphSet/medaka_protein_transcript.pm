package Bio::EnsEMBL::GlyphSet::medaka_protein_transcript;
use strict;
use vars qw(@ISA);
...
sub my_label {
    return "Medaka proteins";
}

sub colours {
    my $self = shift;
    my $Config = $self->{'config'}->get('medaka_protein_transcript','colours');
}
...
sub colour {
  ...
  return (@{$colours->{'medaka_protein'}}, $highlight);
}
...
sub href {
  ...
  return ( $self->{'config'}->get('medaka_protein_transcript','_href_only') eq '#tid' && exists $highlights{lc($gene->stable_id())} ) ?
        "#$tid" : 
...
sub gene_href {
  ...
  my $gid = $gene->stable_id();
  return ( $self->{'config'}->get('medaka_protein_transcript','_href_only') eq '#gid' && exists $highlights{lc($gid)} ) ?
...
sub features {
  my ($self) = @_;
  return $self->{'container'}->get_all_Genes( 'medaka_protein' );
}

sub legend {
  my ($self, $colours) = @_;
  return ('medaka_protein', 1010, 
    [ "Medaka protein" => $colours->{'medaka_protein'}[0], ]
  );
}

sub error_track_name { return "Medaka proteins"; }
