package Bio::EnsEMBL::GlyphSet::medaka_genome_project_transcript;
use strict;
use vars qw(@ISA);
...
sub my_label {
    return 'Medaka Medaka GP trans.';
}

sub colours {
    my $self = shift;
    my $Config = $self->{'config'}->get('medaka_genome_project_transcript','colours');
}
...
sub colour {
  ...
  my $highlight = undef;
  my $colour = $colours->{'genome_project'};
...
sub href {
  ...
  return ( $self->{'config'}->get('medaka_genome_project_transcript','_href_only') eq '#tid' && exists $highlights{lc($gene->stable_id())} ) ?
        "#$tid" : 
...
sub gene_href {
  ...
  my $gid = $gene->stable_id();
  return ( $self->{'config'}->get('medaka_genome_project_transcript','_href_only') eq '#gid' && exists $highlights{lc($gid)} ) ?
...
sub features {
  my ($self) = @_;
  my $track = 'medaka_genome_project_transcript';
  ...
  my @genes;
  foreach my $analysis( 'medaka_genome_project' ){
...
sub legend {
  my ($self, $colours) = @_;
  return ('medaka_genome_project', 1002, 
    [ 'Medaka Medaka GP genes' => $colours->{'genomewise'}[0], ]
  );
}

sub error_track_name { return 'Medaka Medaka GP transcripts'; }
