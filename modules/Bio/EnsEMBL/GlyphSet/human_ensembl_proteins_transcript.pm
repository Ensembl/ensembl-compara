...
sub colours {
    my $self = shift;
    my $Config = $self->{'config'}->get('human_ensembl_proteins_transcript','colours');
}
...
sub gene_href {
  ...
  my $gid = $gene->stable_id();
  return ( $self->{'config'}->get('human_ensembl_proteins_transcript','_href_only') eq '#gid' && exists $highlights{lc($gid)} ) ?
...
sub features {
  my ($self) = @_;
  my $track = 'human_ensembl_proteins_transcript';
  my $slice = $self->{'container'};
  my @genes;
  foreach my $analysis( 'human_ensembl_proteins' ){
...
sub legend {
  my ($self, $colours) = @_;
  return ('human_ensembl_proteins', 1000, 
    [ 'Human genes' => $colours->{'human_ensembl_proteins'}[0], ]
  );
}
