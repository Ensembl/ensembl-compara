package Bio::EnsEMBL::GlyphSet::ciona_kyotograil_2004_transcript;
use strict;
use vars qw(@ISA);
...
sub my_label {
    return "Kyotograil '04 model";
}

sub colours {
    my $self = shift;
    my $Config = $self->{'config'}->get('ciona_kyotograil_2004_transcript','colours');
}
...
sub href {
  ...
  return ( $self->{'config'}->get('ciona_kyotograil_2004_transcript','_href_only') eq '#tid' && exists $highlights{lc($gene->stable_id())} ) ?
        "#$tid" : 
...
sub gene_href {
  ...
  my $gid = $gene->stable_id();
  return ( $self->{'config'}->get('ciona_kyotograil_2004_transcript','_href_only') eq '#gid' && exists $highlights{lc($gid)} ) ?
...
sub features {
  my ($self) = @_;
  return $self->{'container'}->get_all_Genes( 'ciona_kyotograil_2004' );
}

sub legend {
  my ($self, $colours) = @_;
  return ('ciona_genes_11', 1011, 
    [ "Kyotograil '04 model" => $colours->{'kyotograil_2004'}[0], ]
  );
}

sub error_track_name { return "Kyotograil '04 model"; }
