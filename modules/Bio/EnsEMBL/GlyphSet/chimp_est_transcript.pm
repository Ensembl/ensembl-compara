package Bio::EnsEMBL::GlyphSet::chimp_est_transcript;
use strict;
use vars qw(@ISA);
...
sub my_label {
    return 'Chimp EST trans.';
}

sub colours {
    my $self = shift;
    my $Config = $self->{'config'}->get('chimp_est_transcript','colours');
}
...
sub href {
  ...
  return ( $self->{'config'}->get('chimp_est_transcript','_href_only') eq '#tid' && exists $highlights{lc($gene->stable_id())} ) ?
        "#$tid" : 
...
sub gene_href {
  ...
  my $gid = $gene->stable_id();
  return ( $self->{'config'}->get('chimp_est_transcript','_href_only') eq '#gid' && exists $highlights{lc($gid)} ) ?
...
sub legend {
  my ($self, $colours) = @_;
  return ('chimp_est_genes', 1002, 
    [ 'Chimp EST genes' => $colours->{'genomewise'}[0], ]
  );
}

sub error_track_name { return 'Chimp EST transcripts'; }
