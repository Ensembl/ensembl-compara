package Bio::EnsEMBL::GlyphSet::gff_prediction_transcript;
use strict;
use vars qw(@ISA);
...
sub colours {
    my $self = shift;
    my $Config = $self->{'config'}->get('gff_prediction_transcript','colours');
}
...
sub colour {
  ...
  return (@{$colours->{'gff_prediction'}}, $highlight);
}
...
sub href {
  ...
  return ( $self->{'config'}->get('gff_prediction_transcript','_href_only') eq '#tid' && exists $highlights{lc($gene->stable_id())} ) ?
        "#$tid" : 
...
sub gene_href {
  ...
  my $gid = $gene->stable_id();
  return ( $self->{'config'}->get('gff_prediction_transcript','_href_only') eq '#gid' && exists $highlights{lc($gid)} ) ?
...
sub features {
  my ($self) = @_;
  return $self->{'container'}->get_all_Genes( 'gff_prediction' );
}

sub legend {
  my ($self, $colours) = @_;
  return ('gff_prediction', 1010, 
    [ "MPG gene" => $colours->{'gff_prediction'}[0], ]
  );
}

sub error_track_name { return "MPG genes"; }
