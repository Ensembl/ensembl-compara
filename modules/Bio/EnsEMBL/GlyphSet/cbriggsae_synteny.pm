package Bio::EnsEMBL::GlyphSet::cbriggsae_synteny;
use strict;
use vars qw(@ISA);
use Bio::EnsEMBL::GlyphSet_simple;
@ISA = qw(Bio::EnsEMBL::GlyphSet_simple);

sub my_label { return "C.briggsae synteny"; }

sub features {
    my ($self) = @_;
    return $self->{'container'}->get_all_compara_Syntenies( 'Caenorhabditis briggsae' );
}
...
sub href {
    ...
    my $en = $st + 1e6;
    return "/Caenorhabditis_briggsae/$ENV{'ENSEMBL_SCRIPT'}?l=".$f->{'hit_chr_name'}.":$st-$en";
}
...
sub zmenu {
    ...
    my $zmenu = { 
        ...
        '03:Jump to C.briggsae' => $self->href( $f )
    };
    return $zmenu;
}
