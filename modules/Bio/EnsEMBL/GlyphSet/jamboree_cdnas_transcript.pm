package Bio::EnsEMBL::GlyphSet::jamboree_cdnas_transcript;
use strict;
use vars qw(@ISA);
...
sub my_label {
    return "X.trop. jambo. genes";
}

sub colours {
    my $self = shift;
    my $Config = $self->{'config'}->get('jamboree_cdnas_transcript','colours');
}
...
sub href {
  ...
  return ( $self->{'config'}->get('jamboree_cdnas_transcript','_href_only') eq '#tid' && exists $highlights{lc($gene->stable_id())} ) ?
        "#$tid" : 
...
sub gene_href {
  ...
  my $gid = $gene->stable_id();
  return ( $self->{'config'}->get('jamboree_cdnas_transcript','_href_only') eq '#gid' && exists $highlights{lc($gid)} ) ?
...
sub features {
  my ($self) = @_;
  return $self->{'container'}->get_all_Genes( 'jamboree_CDNAs' );
}

sub legend { return }

sub error_track_name { return "X.trop. jambo. genes"; }
