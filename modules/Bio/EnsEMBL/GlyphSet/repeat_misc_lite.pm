...
sub _init {
    ...
    my $max_length     = $Config->get( 'repeat_misc_lite', 'threshold' ) || 2000;
    my $navigation     = $Config->get( 'repeat_misc_lite', 'navigation' ) || 'off';
    my $max_length_nav = $Config->get( 'repeat_misc_lite', 'navigation_threshold' ) || 1500;
    my $feature_colour = $Config->get( 'repeat_misc_lite', 'col' );
	...
	my $repeats = $vc->dbobj->get_LiteAdaptor->fetch_virtualRepeatFeatures_start_end(
		$vc->_chr_name, $vc->_global_start, $vc->_global_end, 'misc', $self->glob_bp() 
	);

    unless(@$repeats>0 || $Config->get('_settings','opt_empty_tracks')==0 ) {
        $self->errorTrack("No misc. repeats in this region");
        return;
    }
