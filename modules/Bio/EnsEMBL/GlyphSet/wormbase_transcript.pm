package Bio::EnsEMBL::GlyphSet::wormbase_transcript;
use strict;
use vars qw(@ISA);
...
sub colours {
  ...
  return $Config->get('wormbase_transcript','colours');
}

sub features {
  my ($self) = @_;
  my $track = 'wormbase_transcript';
  my $slice = $self->{'container'};
  my $sp = $self->{'_config_file_name_'};
  my @analyses = ( 'wormbase', 'pseudogene');
...
sub href {
  ...
  return ( $self->{'config'}->get('wormbase_transcript','_href_only') eq '#tid' && exists $highlights{lc($gene->stable_id)} ) ?
    "#$tid" : 
...
sub gene_href {
  ...
  my $script_name = $ENV{'ENSEMBL_SCRIPT'} eq 'genesnpview' ? 'genesnpview' : 'geneview';
  return ( $self->{'config'}->get('wormbase_transcript','_href_only') eq '#gid' && exists $highlights{lc($gene->stable_id)} ) ?
    "#$gid" :
    qq(/@{[$self->{container}{_config_file_name_}]}/$script_name?gene=$gid);
