...
sub _init {
  ...
  foreach my $box (@$synteny_data) {
    ...
    } else { # We have a hit which is on a chr which we have to position
    ...
    if ($SIDE) {
      my $marked = ($main_dfr->dnafrag_start <= $self->{'container'}->{'line'} && $self->{'container'}->{'line'} <= $main_dfr->dnafrag_end) ? $SIDE : 0;
  ...
  foreach my $chr2 (@chromosomes) {
    ...
    my ($h_offset2, $v_offset2) = $flag == 0 ?
      ($h_offset + $N/2 * ($secondary_length + $spacing), $outer_padding) : # LHS
      ($h_offset + ($N-$FLAG)/2 * ($secondary_length + $spacing), 2 * $inner_padding + $secondary_width + $main_width + $outer_padding); # RHS
...
sub draw_chromosome {
  ...
  if (@bands) {
    @ends = (( $bands[0]->stain eq 'tip' ? () : 1 ), ( $bands[-1]->stain eq 'tip' ? () : -1 ));
  } else {
    @ends = (-1,1);
  }
