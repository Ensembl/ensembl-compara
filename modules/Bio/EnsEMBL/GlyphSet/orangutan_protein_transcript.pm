package Bio::EnsEMBL::GlyphSet::orangutan_protein_transcript;
use strict;
use vars qw(@ISA);
...
sub my_label {
    return 'Orangutan trans.';
}

sub colours {
    my $self = shift;
    my $Config = $self->{'config'}->get('orangutan_protein_transcript','colours');
}
...
sub colour {
  ...
  return (@{$colours->{'orangutan_protein'}}, $highlight);
}
...
sub href {
  ...
  return ( $self->{'config'}->get('orangutan_protein_transcript','_href_only') eq '#tid' && exists $highlights{lc($gene->stable_id())} ) ?
        "#$tid" : 
...
sub gene_href {
  ...
  my $gid = $gene->stable_id();
  return ( $self->{'config'}->get('orangutan_protein_transcript','_href_only') eq '#gid' && exists $highlights{lc($gid)} ) ?
...
sub features {
  my ($self) = @_;
  my $track = 'orangutan_protein_transcript';
  my $slice = $self->{'container'};
  my @genes;
  foreach my $analysis( 'orangutan_protein' ){
warn "... $analysis ...";
...
sub legend {
  my ($self, $colours) = @_;
  return ('orangutan_protein', 1000, 
    [ 'Orangutan genes' => $colours->{'orangutan_protein'}[0], ]
  );
}

sub error_track_name { return 'Orangutan proteins'; }
