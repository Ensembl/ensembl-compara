package Bio::EnsEMBL::GlyphSet::human_cdna_transcript;
use strict;
use vars qw(@ISA);
...
sub my_label {
    return 'Human cDNA trans.';
}

sub colours {
    my $self = shift;
    my $Config = $self->{'config'}->get('human_cdna_transcript','colours');
}
...
sub href {
  ...
  return ( $self->{'config'}->get('human_cdna_transcript','_href_only') eq '#tid' && exists $highlights{lc($gene->stable_id())} ) ?
        "#$tid" : 
...
sub gene_href {
  ...
  my $gid = $gene->stable_id();
  return ( $self->{'config'}->get('human_cdna_transcript','_href_only') eq '#gid' && exists $highlights{lc($gid)} ) ?
...
sub features {
  my ($self) = @_;
  my $track = 'human_cdna_transcript';
  ...
  my @genes;
  foreach my $analysis( 'human_cdna' ){
...
sub legend {
  my ($self, $colours) = @_;
  return ('human_cdna_genes', 1002, 
    [ 'Human cDNA genes' => $colours->{'genomewise'}[0], ]
  );
}

sub error_track_name { return 'Human cDNA transcripts'; }
