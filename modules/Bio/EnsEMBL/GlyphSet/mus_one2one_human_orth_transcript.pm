package Bio::EnsEMBL::GlyphSet::mus_one2one_human_orth_transcript;
use strict;
use vars qw(@ISA);
...
sub colours {
    my $self = shift;
    my $Config = $self->{'config'}->get('mus_one2one_human_orth_transcript','colours');
}
...
sub colour {
  ...
  return (@{$colours->{'mus_one2one_human_orth'}}, $highlight);
}
...
sub href {
  ...
  return ( $self->{'config'}->get('mus_one2one_human_orth_transcript','_href_only') eq '#tid' && exists $highlights{lc($gene->stable_id())} ) ?
        "#$tid" : 
...
sub gene_href {
  ...
  my $gid = $gene->stable_id();
  return ( $self->{'config'}->get('mus_one2one_human_orth_transcript','_href_only') eq '#gid' && exists $highlights{lc($gid)} ) ?
...
sub features {
  my ($self) = @_;
  my $track = 'mus_one2one_human_orth_transcript';
  my $slice = $self->{'container'};
  my @genes;
  foreach my $analysis( 'mus_one2one_human_orth' ){
...
sub legend {
  my ($self, $colours) = @_;
  return ('mus_one2one_human_orth', 1000, 
    [ 'Mm/Hs Genes' => $colours->{'mus_one2one_human_orth'}[0], ]
  );
}

sub error_track_name { return 'Mm/Hs Proteins'; }
