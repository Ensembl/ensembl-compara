package Bio::EnsEMBL::GlyphSet::platypus_olfactory_receptors_transcript;
use strict;
use vars qw(@ISA);
...
sub my_label {
    return 'Olfactory receptor genes';
}

sub colours {
    my $self = shift;
    my $Config = $self->{'config'}->get('platypus_olfactory_receptors_transcript','colours');
}
...
sub colour {
  ...
  return (@{$colours->{'olfactory'}}, $highlight);
}
...
sub href {
  ...
  return ( $self->{'config'}->get('platypus_olfactory_receptors_transcript','_href_only') eq '#tid' && exists $highlights{lc($gene->stable_id())} ) ?
        "#$tid" : 
...
sub gene_href {
  ...
  my $gid = $gene->stable_id();
  return ( $self->{'config'}->get('platypus_olfactory_receptors_transcript','_href_only') eq '#gid' && exists $highlights{lc($gid)} ) ?
...
sub zmenu {
  ...
  my $zmenu = {
    'caption'         => "Olfactory Receptor Gene" ,
...
sub features {
  my ($self) = @_;
  my $track = 'platypus_olfactory_receptors_transcript';
  my $slice = $self->{'container'};
  my @genes;
  foreach my $analysis( 'platypus_olfactory_receptors' ){
...
sub legend {
  my ($self, $colours) = @_;
  return ('platypus_olfactory_receptors', 1000, 
    [ 'Olfactory receptor genes' => $colours->{'olfactory'}[0], ]
  );
}

sub error_track_name { return 'Olfactory Receptor Genes'; }
