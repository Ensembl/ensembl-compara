package Bio::EnsEMBL::GlyphSet::ciona_jgi_v1_transcript;
use strict;
use vars qw(@ISA);
...
sub my_label {
    return "JGI 1.0 models";
}

sub colours {
    my $self = shift;
    my $Config = $self->{'config'}->get('ciona_jgi_v1_transcript','colours');
}
...
sub href {
  ...
  return ( $self->{'config'}->get('ciona_jgi_v1_transcript','_href_only') eq '#tid' && exists $highlights{lc($gene->stable_id())} ) ?
        "#$tid" : 
...
sub gene_href {
  ...
  my $gid = $gene->stable_id();
  return ( $self->{'config'}->get('ciona_jgi_v1_transcript','_href_only') eq '#gid' && exists $highlights{lc($gid)} ) ?
...
sub features {
  my ($self) = @_;
  return $self->{'container'}->get_all_Genes( 'ciona_jgi_v1' );
}

sub legend {
  my ($self, $colours) = @_;
  return ('ciona_genes_10', 1010, 
    [ "JGI 1.0 models" => $colours->{'jgi_v1'}[0], ]
  );
}

sub error_track_name { return "JGI 1.0 models"; }
