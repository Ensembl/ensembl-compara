package Bio::EnsEMBL::GlyphSet::oxford_fgu_ext_transcript;
use strict;
use vars qw(@ISA);
...
sub zmenu {
  ...
  my $zmenu = {
    ...
    "01:Gene:$gid"    => "/@{[$self->{container}{_config_file_name_}]}/geneview?gene=$gid;db=otherfeatures",
    "02:Transcr:$tid" => "/@{[$self->{container}{_config_file_name_}]}/transview?transcript=$tid;db=otherfeatures",                	
  ...
  if($pid) {
    $zmenu->{"03:Peptide:$pid"}   = qq(/@{[$self->{container}{_config_file_name_}]}/protview?peptide=$pid;db=otherfeatures);
...
sub features {
  ...
  foreach my $analysis( 'oxford_fgu' ){
    push @genes, @{ $slice->get_all_Genes( $analysis, 'otherfeatures' ) }
  }
  return [@genes];
}
