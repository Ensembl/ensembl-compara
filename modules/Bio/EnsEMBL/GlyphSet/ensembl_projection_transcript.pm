package Bio::EnsEMBL::GlyphSet::ensembl_projection_transcript;
use strict;
...
sub colours {
  ...
  return $Config->get('ensembl_projection_transcript','colours');
}

sub features {
  my ($self) = @_;
  my $track = 'ensembl_projection_transcript';
  my $slice = $self->{'container'};
  my $sp = $self->{'_config_file_name_'};
  my @analyses = ( 'ensembl_projection' );
...
sub href {
  ...
  return ( $self->{'config'}->get('ensembl_projection_transcript','_href_only') eq '#tid' && exists $highlights{lc($gene->stable_id())} ) ?
    "#$tid" : 
...
sub gene_href {
  ...
  my $script_name = $ENV{'ENSEMBL_SCRIPT'} eq 'genesnpview' ? 'genesnpview' : 'geneview';
  return ( $self->{'config'}->get('ensembl_projection_transcript','_href_only') eq '#gid' && exists $highlights{lc($gene->stable_id)} ) ?
    "#$gid" :
    qq(/@{[$self->{container}{_config_file_name_}]}/$script_name?gene=$gid);
