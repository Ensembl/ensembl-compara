package Bio::EnsEMBL::GlyphSet::medaka_transcriptcoalescer_transcript;
use strict;
use vars qw(@ISA);
...
sub colours {
    my $self = shift;
    my $Config = $self->{'config'}->get('medaka_transcriptcoalescer_transcript','colours');
}
...
sub colour {
  ...
  my $highlight = undef;
  my $colour = $colours->{'transcriptcoalescer'};
...
sub href {
  ...
  return ( $self->{'config'}->get('medaka_transcriptcoalescer_transcript','_href_only') eq '#tid' && exists $highlights{lc($gene->stable_id())} ) ?
        "#$tid" : 
...
sub gene_href {
  ...
  my $gid = $gene->stable_id();
  return ( $self->{'config'}->get('medaka_transcriptcoalescer_transcript','_href_only') eq '#gid' && exists $highlights{lc($gid)} ) ?
...
sub zmenu {
  ...
  my $zmenu = {
    'caption'         => "Medaka TransCoalescer Gene",
...
sub features {
  my ($self) = @_;
  my $track = 'medaka_transcriptcoalescer_transcript';
  ...
  my @genes;
  foreach my $analysis( 'medaka_transcriptcoalescer' ){
...
sub legend {
  my ($self, $colours) = @_;
  return ('medaka_transcriptcoalescer', 1002, 
    [ 'Medaka TransCoalescer genes' => $colours->{'genomewise'}[0], ]
  );
}

sub error_track_name { return 'Medaka TransCoalescer transcripts'; }
