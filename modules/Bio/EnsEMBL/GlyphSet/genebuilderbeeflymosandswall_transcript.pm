...
sub href {
  ...
  return ( $self->{'config'}->get('genebuilderbeeflymosandswall_transcript','_href_only') eq '#tid' && exists $highlights{$gene->stable_id()} ) ?
    "#$tid" : 
...
sub gene_href {
  ...
  my $script_name = $ENV{'ENSEMBL_SCRIPT'} eq 'genesnpview' ? 'genesnpview' : 'geneview';
  return ( $self->{'config'}->get('genebuilderbeeflymosandswall_transcript','_href_only') eq '#gid' && exists $highlights{$gene->stable_id()} ) ?
    "#$gid" :
    qq(/@{[$self->{container}{_config_file_name_}]}/$script_name?gene=$gid);
