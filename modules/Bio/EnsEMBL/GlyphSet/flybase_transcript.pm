package Bio::EnsEMBL::GlyphSet::flybase_transcript;
use strict;
use vars qw(@ISA);
use EnsWeb;
...
sub my_label {
  my $self = shift;
  return $self->{'config'}->{'_draw_single_Transcript'} || $self->{'config'}->{'geneid'} || "Flybase trans.";
}

sub colours {
  my $self = shift;
  my $Config = $self->{'config'};
  return $Config->get('flybase_transcript','colours');
}

sub features {
  my ($self) = @_;
  my $track = 'flybase_transcript';
  my $slice = $self->{'container'};
  my $sp = $self->{'_config_file_name_'};
  my @analyses = ( 'flybase', 'pseudogene');
...
sub href {
  ...
  return ( $self->{'config'}->get('flybase_transcript','_href_only') eq '#tid' && exists $highlights{lc($gene->stable_id)} ) ?
    "#$tid" : 
...
sub gene_href {
  ...
  my $script_name = $ENV{'ENSEMBL_SCRIPT'} eq 'genesnpview' ? 'genesnpview' : 'geneview';
  return ( $self->{'config'}->get('flybase_transcript','_href_only') eq '#gid' && exists $highlights{lc($gene->stable_id)} ) ?
    "#$gid" :
    qq(/@{[$self->{container}{_config_file_name_}]}/$script_name?gene=$gid);
...
sub legend {
  my ($self, $colours) = @_;
  return ('genes', 900, [
    'Flybase predicted genes (known)' => $colours->{'_KNOWN'}[0],
    'Flybase predicted genes (novel)' => $colours->{'_'}[0],
    'Flybase pseudogenes'             => $colours->{'_PSEUDO'}[0],
  ]);
}

sub error_track_name { return 'Flybase transcripts'; }
