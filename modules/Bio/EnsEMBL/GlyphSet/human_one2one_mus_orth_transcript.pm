package Bio::EnsEMBL::GlyphSet::human_one2one_mus_orth_transcript;
use strict;
use vars qw(@ISA);
...
sub colours {
    my $self = shift;
    my $Config = $self->{'config'}->get('human_one2one_mus_orth_transcript','colours');
}
...
sub colour {
  ...
  return (@{$colours->{'human_one2one_mus_orth'}}, $highlight);
}
...
sub href {
  ...
  return ( $self->{'config'}->get('human_one2one_mus_orth_transcript','_href_only') eq '#tid' && exists $highlights{lc($gene->stable_id())} ) ?
        "#$tid" : 
...
sub gene_href {
  ...
  my $gid = $gene->stable_id();
  return ( $self->{'config'}->get('human_one2one_mus_orth_transcript','_href_only') eq '#gid' && exists $highlights{lc($gid)} ) ?
...
sub features {
  my ($self) = @_;
  my $track = 'human_one2one_mus_orth_transcript';
  my $slice = $self->{'container'};
  my @genes;
  foreach my $analysis( 'human_one2one_mus_orth' ){
...
sub legend {
  my ($self, $colours) = @_;
  return ('human_one2one_mus_orth', 1000, 
    [ 'Hs/Mm Genes' => $colours->{'human_one2one_mus_orth'}[0], ]
  );
}
