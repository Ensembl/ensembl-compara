package Bio::EnsEMBL::GlyphSet::dog_protein_transcript;
use strict;
use vars qw(@ISA);
...
sub colours {
    my $self = shift;
    my $Config = $self->{'config'}->get('dog_protein_transcript','colours');
}
...
sub colour {
  ...
  return (@{$colours->{'dog_protein'}}, $highlight);
}
...
sub href {
  ...
  return ( $self->{'config'}->get('dog_protein_transcript','_href_only') eq '#tid' && exists $highlights{lc($gene->stable_id())} ) ?
        "#$tid" : 
...
sub gene_href {
  ...
  my $gid = $gene->stable_id();
  return ( $self->{'config'}->get('dog_protein_transcript','_href_only') eq '#gid' && exists $highlights{lc($gid)} ) ?
...
sub features {
  my ($self) = @_;
  my $track = 'dog_protein_transcript';
  my $slice = $self->{'container'};
  my @genes;
  foreach my $analysis( 'dog_protein' ){
...
sub legend {
  my ($self, $colours) = @_;
  return ('dog_protein', 1000, 
    [ 'Dog genes' => $colours->{'dog_protein'}[0], ]
  );
}

sub error_track_name { return 'Dog proteins'; }
