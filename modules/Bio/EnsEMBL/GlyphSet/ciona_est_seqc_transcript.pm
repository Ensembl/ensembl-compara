package Bio::EnsEMBL::GlyphSet::ciona_est_seqc_transcript;
use strict;
use vars qw(@ISA);
...
sub my_label {
    return "3' EST genes (Kyoto)";
}

sub colours {
    my $self = shift;
    my $Config = $self->{'config'}->get('ciona_est_seqc_transcript','colours');
}
...
sub href {
  ...
  return ( $self->{'config'}->get('ciona_est_seqc_transcript','_href_only') eq '#tid' && exists $highlights{lc($gene->stable_id())} ) ?
        "#$tid" : 
...
sub gene_href {
  ...
  my $gid = $gene->stable_id();
  return ( $self->{'config'}->get('ciona_est_seqc_transcript','_href_only') eq '#gid' && exists $highlights{lc($gid)} ) ?
...
sub features {
  my ($self) = @_;
  return $self->{'container'}->get_all_Genes( 'ciona_est_seqc' );
}

sub legend {
  my ($self, $colours) = @_;
  return ('ciona_genes_1', 1001, 
    [ "3' EST genes (Kyoto)" => $colours->{'est_seqc'}[0], ]
  );
}

sub error_track_name { return "3' EST genes (Kyoto)"; }
