package Bio::EnsEMBL::GlyphSet::vectorbase_0_5_transcript;
use strict;
use vars qw(@ISA);
...
sub my_label {
    return 'VectorBase trans.';
}

sub colours {
    my $self = shift;
    my $Config = $self->{'config'}->get('vectorbase_0_5_transcript','colours');
}
...
sub colour {
  ...
  return (@{$colours->{'vectorbase_0_5'}}, $highlight); 
}
...
sub href {
  ...
  return ( $self->{'config'}->get('vectorbase_0_5_transcript','_href_only') eq '#tid' && exists $highlights{lc($gene->stable_id())} ) ?
        "#$tid" : 
...
sub gene_href {
  ...
  my $gid = $gene->stable_id();
  return ( $self->{'config'}->get('vectorbase_0_5_transcript','_href_only') eq '#gid' && exists $highlights{lc($gid)} ) ?
...
sub features {
  my ($self) = @_;
  my $track = 'vectorbase_0_5_transcript';
  my $slice = $self->{'container'};
  my @genes;
  foreach my $analysis( 'vectorbase_0_5' ){
...
sub legend {
  my ($self, $colours) = @_;
  return ('vectorbase_0_5', 1000, 
    [ 'VectorBase genes' => $colours->{'vectorbase_0_5'}[0], ]
  );
}

sub error_track_name { return 'VectorBase proteins'; }
