package Bio::EnsEMBL::GlyphSet::oxford_genes_transcript;
use strict;
use vars qw(@ISA);
...
sub colours {
    my $self = shift;
    my $Config = $self->{'config'}->get('oxford_genes_transcript','colours');
}
...
sub colour {
  ...
  return (@{$colours->{'oxford'}}, $highlight);
}
...
sub href {
  ...
  return ( $self->{'config'}->get('oxford_genes_transcript','_href_only') eq '#tid' && exists $highlights{lc($gene->stable_id())} ) ?
        "#$tid" : 
...
sub gene_href {
  ...
  my $gid = $gene->stable_id();
  return ( $self->{'config'}->get('oxford_genes_transcript','_href_only') eq '#gid' && exists $highlights{lc($gid)} ) ?
...
sub features {
  my ($self) = @_;
  my $track = 'oxford_genes_transcript';
  my $slice = $self->{'container'};
  my @genes;
  foreach my $analysis( 'oxford_genes' ){
...
sub legend {
  my ($self, $colours) = @_;
  return ('oxford_genes', 1000, 
    [ 'Oxford genes' => $colours->{'oxford_genes'}[0], ]
  );
}

sub error_track_name { return 'Oxford genes'; }
