...
print STDERR "Target = $target\n";
    ...
    GENE: for my $eg (@allgenes) {
    	my $vgid = $eg->id();
        my $hi_colour = $Config->get('transcript','hi') if(defined $highlights && $highlights =~ /\|$vgid\|/);
	$type = $eg->type();
	#print STDERR "type: $type\n";
	
      TRANSCRIPT: for my $transcript ($eg->each_Transcript()) {
	  print STDERR "tr: ", $transcript->id(), " ==> $target\n";
	  
	  next unless ($transcript->id() eq $target);
	  #########
