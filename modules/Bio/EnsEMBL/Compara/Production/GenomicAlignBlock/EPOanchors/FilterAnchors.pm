...
package Bio::EnsEMBL::Compara::Production::GenomicAlignBlock::EPOanchors::FilterAnchors;
...
sub run {
	...
	$self->anchor_ids_with_zero_strand(
		$anchor_align_adaptor->fetch_all_anchors_with_zero_strand(
			$self->genome_db_ids, $self->test_method_link_species_set_id));
	...
	foreach my $anchor (@{$all_anchors}) {
		my $dnafrags_and_genomedbs = $anchor_align_adaptor->fetch_dnafrag_and_genome_db_ids_by_test_mlssid_and_genome_db_ids_and_anchor_id(
			$self->test_method_link_species_set_id, $self->genome_db_ids, $anchor->[0]);
