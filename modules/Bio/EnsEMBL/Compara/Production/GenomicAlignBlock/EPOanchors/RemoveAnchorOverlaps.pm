...
Bio::EnsEMBL::Compara::Production::GenomicAlignBlock::EPOanchors::RemoveAnchorOverlaps
...
package Bio::EnsEMBL::Compara::Production::GenomicAlignBlock::EPOanchors::RemoveAnchorOverlaps;
...
sub run {
	...
	my $dnafrag_ids = $anchor_align_adaptor->fetch_all_dnafrag_ids($self->genome_db_ids);
	...
	foreach my $genome_db_id(sort keys %{$dnafrag_ids}) {
		foreach my $dnafrag_id(@{$dnafrag_ids->{$genome_db_id}}) {	
			my @Dnafrag_anchors = @{$anchor_align_adaptor->fetch_all_anchors_by_dnafrag_id_and_test_mlssid_and_genomedb_list($dnafrag_id,
