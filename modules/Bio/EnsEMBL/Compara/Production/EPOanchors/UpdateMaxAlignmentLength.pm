...
Bio::EnsEMBL::Compara::Production::EPOanchors::UpdateMaxAlignmentLength
...
package Bio::EnsEMBL::Compara::Production::EPOanchors::UpdateMaxAlignmentLength;
...
#    my $sql_gag_to_exec = $sql_gag . "(" . join(",", @ga_ids) . ")";
    ...
    foreach my $sql ($sql_ga_to_exec,$sql_gab_to_exec) {#,$sql_gag_to_exec) {
      my $sth = $dba->dbc->prepare($sql);
      $sth->execute;
      $sth->finish;
    }
