...
This module is intended to access data in the nc_tree_node and
nc_tree_member tables. These tables store nc trees including
phylogenetic topology and multiple sequence alignments
...
=head2 fetch_by_Member_root_id
  ...
  Example    : $nctree = $nctree_adaptor->fetch_by_Member_root_id($member);

  Description: Fetches from the database the nc_tree that contains the member
  Returntype : Bio::EnsEMBL::Compara::NCTree
  Exceptions :
  Caller     :
...
sub fetch_by_Member_root_id {
  ...
  my $nc_tree = $self->fetch_node_by_node_id($node->node_id);
...
=head2 fetch_AlignedMember_by_member_id_root_id
  ...
  Example    :
      ...
      my $aligned_member = $nctree_adaptor->
...
sub gene_member_id_is_in_tree {
  ...
  my $sth = $self->prepare("SELECT ptm1.root_id FROM member m1, nc_tree_member ptm1 WHERE ptm1.member_id=m1.member_id AND m1.gene_member_id=? LIMIT 1");
...
sub delete_tag {
  ...
  my $sql = "DELETE from nc_tree_tag where node_id=$node_id and tag=\"$tag\"";
  #print("$sql\n");
  $self->dbc->do($sql);
}
...
sub store_node {
  ...
  my $sth = $self->prepare("INSERT INTO nc_tree_node 
                             (parent_id,
                              root_id,
                              clusterset_id,
                              left_index,
...
sub update_node {
  ...
  my $sth = $self->prepare("UPDATE nc_tree_node SET
                            parent_id=?,
                            root_id=?,
                            left_index=?,
                            right_index=?,
...
sub merge_nodes {
  ...
  $sth = $self->prepare("DELETE from nc_tree_node WHERE node_id=?");
...
sub delete_flattened_leaf {
  ...
  $self->dbc->do("DELETE from nc_tree_node WHERE node_id = $node_id");
  $self->dbc->do("DELETE from nc_tree_tag WHERE node_id = $node_id");
  $self->dbc->do("DELETE from nc_tree_member WHERE node_id = $node_id");
}
...
sub delete_node {
  ...
  $self->dbc->do("UPDATE nc_tree_node dn, nc_tree_node n SET ". 
...
sub delete_node_and_under {
  ...
  foreach my $subnode (@all_subnodes) {
    ...
    $self->dbc->do("DELETE from nc_tree_node WHERE node_id = $subnode_id");
    $self->dbc->do("DELETE from nc_tree_tag WHERE node_id = $subnode_id");
    $self->dbc->do("DELETE from nc_tree_member WHERE node_id = $subnode_id");
  }
  my $node_id = $node->node_id;
  $self->dbc->do("DELETE from nc_tree_node WHERE node_id = $node_id");
  $self->dbc->do("DELETE from nc_tree_tag WHERE node_id = $node_id");
...
sub _load_tagvalues {
  ...
  my $sth = $self->prepare("SELECT tag,value from nc_tree_tag where node_id=?");
...
sub tables {
  my $self = shift;
  return [['nc_tree_node', 't']];
}

sub left_join_clause {
  return "left join nc_tree_member tm on t.node_id = tm.node_id left join member m on tm.member_id = m.member_id";
}
