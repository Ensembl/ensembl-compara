...
sub fetch_AlignedMember_by_member_id_root_id {
  ...
  my $join = [[['protein_tree_member', 'tm'], 
               't.nestedset_id = tm.nestedset_id',
               ['tm.cigar_line']],
              [['member', 'm'], 
...
sub store {
  ...
  $node->nestedset_id( $sth->{'mysql_insertid'} );
  $node->adaptor($self);
  $sth->finish;
  ...
  if($node->isa('Bio::EnsEMBL::Compara::AlignedMember')) {
    $sth = $self->prepare("INSERT ignore INTO protein_tree_member 
                               (nestedset_id,
                                member_id,
                                cigar_line)  VALUES (?,?,?)");
    $sth->execute($node->nestedset_id, $node->member_id, $node->cigar_line);
    $sth->finish;
  }
...
sub merge_nodes {
  ...
  # printf("MERGE children from parent %d => %d\n", $node2->nestedset_id, $node1->nestedset_id);
  
  my $sth = $self->prepare("UPDATE protein_tree_nodes SET
                              parent_id=?,
                              root_id=?
			    WHERE parent_id=?");
  $sth->execute($node1->nestedset_id, $node1->root->nestedset_id, $node2->nestedset_id);
  $sth->finish;
  
  $sth = $self->prepare("DELETE from protein_tree_nodes WHERE nestedset_id=?");
  $sth->execute($node2->nestedset_id);
  $sth->finish;
}
...
sub left_join_clause {
  return "left join protein_tree_member tm using (nestedset_id) left join member m using(member_id)";
}
