...
=head1 SYNOPSIS
  ...
  my $supertree_adaptor = $reg->get_adaptor("Multi", "compara", "SuperProteinTree");
...
This module is intended to access data in the super_protein_tree_node
and super_protein_tree_member tables. These tables store large protein
trees in the form of multiple sequence alignments.
...
sub delete_tag {
  ...
  my $sql = "DELETE from super_protein_tree_tag where node_id=$node_id and tag=\"$tag\"";
  #print("$sql\n");
  $self->dbc->do($sql);
}
...
sub store_node {
  ...
  my $sth = $self->prepare("INSERT INTO super_protein_tree_node 
                             (parent_id,
                              root_id,
                              clusterset_id,
                              left_index,
...
sub update_node {
  ...
  my $sth = $self->prepare("UPDATE super_protein_tree_node SET
                            parent_id=?,
                            root_id=?,
                            left_index=?,
                            right_index=?,
...
sub merge_nodes {
  ...
  $sth = $self->prepare("DELETE from super_protein_tree_node WHERE node_id=?");
...
#             "n.parent_id = dn.parent_id WHERE n.parent_id=dn.node_id AND dn.node_id=$node_id");
  $self->dbc->do("DELETE from super_protein_tree_node WHERE node_id = $node_id");
  $self->dbc->do("DELETE from super_protein_tree_tag WHERE node_id = $node_id");
  $self->dbc->do("DELETE from super_protein_tree_member WHERE node_id = $node_id");
}
...
sub delete_node {
  ...
  my $node_id = $node->node_id;
  #print("delete node $node_id\n");
  $self->dbc->do("UPDATE super_protein_tree_node dn, super_protein_tree_node n SET ". 
            "n.parent_id = dn.parent_id WHERE n.parent_id=dn.node_id AND dn.node_id=$node_id");
  $self->dbc->do("DELETE from super_protein_tree_node WHERE node_id = $node_id");
  $self->dbc->do("DELETE from super_protein_tree_tag WHERE node_id = $node_id");
  $self->dbc->do("DELETE from super_protein_tree_member WHERE node_id = $node_id");
...
sub delete_node_and_under {
  ...
  foreach my $subnode (@all_subnodes) {
    ...
    $self->dbc->do("DELETE from super_protein_tree_node WHERE node_id = $subnode_id");
    $self->dbc->do("DELETE from super_protein_tree_tag WHERE node_id = $subnode_id");
    $self->dbc->do("DELETE from super_protein_tree_member WHERE node_id = $subnode_id");
  }
  my $node_id = $node->node_id;
  $self->dbc->do("DELETE from super_protein_tree_node WHERE node_id = $node_id");
  $self->dbc->do("DELETE from super_protein_tree_tag WHERE node_id = $node_id");
...
sub _load_tagvalues {
  ...
  my $sth = $self->prepare("SELECT tag,value from super_protein_tree_tag where node_id=?");
...
sub _store_tagvalue {
  ...
  my $sth = $self->prepare("INSERT ignore into super_protein_tree_tag (node_id,tag) values (?, ?)");
  $sth->execute($node_id, $tag);
  #my $sql = "INSERT ignore into super_protein_tree_tag (node_id,tag) values ($node_id,\"$tag\")";
  #print("$sql\n");
  #$self->dbc->do($sql);

  $sth = $self->prepare("UPDATE super_protein_tree_tag set value=? where node_id=? and tag=?");
  $sth->execute($value, $node_id, $tag);
  #$sql = "UPDATE super_protein_tree_tag set value=\"$value\" where node_id=$node_id and tag=\"$tag\"";
  #print("$sql\n");
  #$self->dbc->do($sql);
}
...
sub tables {
  my $self = shift;
  return [['super_protein_tree_node', 't']];
}

sub left_join_clause {
  return "left join super_protein_tree_member tm on t.node_id = tm.node_id left join member m on tm.member_id = m.member_id";
}
