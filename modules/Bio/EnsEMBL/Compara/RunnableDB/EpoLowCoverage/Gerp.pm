...
Bio::EnsEMBL::Compara::RunnableDB::EpoLowCoverage::Gerp
...
package Bio::EnsEMBL::Compara::RunnableDB::EpoLowCoverage::Gerp;
...
sub fetch_input {
  ...
  if (scalar(@$gas) > 2) {
	  ...
	  my $gata = $self->param('comparaDBA')->get_GenomicAlignTreeAdaptor;
	  ...
	  my $mod_tree_file = $self->worker_temp_directory . $TREE_FILE;
	  $self->param('modified_tree_file', $mod_tree_file);

          open (TREE, ">".$self->param('modified_tree_file')) or throw "error writing alignment ($mod_tree_file) file\n";
	  ...
	  my $cp_cmd = "cp " . $self->param('param_file') . " " . $self->param_file_tmp;
	  unless (system ($cp_cmd) == 0) {
	      throw("error copying " . $self->param('param_file') . " to " . $self->param_file_tmp . "\n");
	  }
...
sub run {
    ...
    $self->param('comparaDBA')->dbc->disconnect_when_inactive(1);  
...
sub write_output {
    ...
    #Start transaction
    #
    my $dbh = $self->param('comparaDBA')->dbc->db_handle;
...
sub param_file_tmp {
  my $self = shift;
  $self->param('_param_file_tmp') = shift if(@_);
  return $self->param('_param_file_tmp');
}
...
sub run_gerp {
	...
	$program_file = "$BIN_DIR{$self->param('program_version')}/GERP.pl";
    }

    throw($program_file . " is not executable Gerp::run ")
	unless ($program_file && -x $program_file);
...
sub run_gerp_v2 {
    ...
    if (defined $self->param('program_file')) {
	$self->program_file($self->param('program_file'));
    } else {
	if (defined($self->analysis) and defined($self->analysis->program_file)) {
...
sub _parse_results_v2 {
  ...
  #generate rates and constraints file names
  #Don't store conservation_scores if no_conservation_scores has been set
...
sub _parse_cons_file {
    ...
    my $mlssa = $self->param('comparaDBA')->get_MethodLinkSpeciesSetAdaptor;
    my $species_set;
    if (!defined $self->param('species_set')) {
	$species_set = $gab->method_link_species_set->species_set;
    } else {
    ...
    my $constrained_element_adaptor = $self->param('comparaDBA')->get_ConstrainedElementAdaptor;
...
sub _parse_rates_file {
    ...
    my $gaba = $self->param('comparaDBA')->get_GenomicAlignBlockAdaptor;
    my $gab = $gaba->fetch_by_dbID($self->param('genomic_align_block_id'));
    my $cs_adaptor = $self->param('comparaDBA')->get_ConservationScoreAdaptor;
...
sub _build_tree_string {
	...
	print "Taking tree from tree_analysis_data. No longer implemented.\n";
	...
	if (-e $tree_file) {
	    ...
	    my $meta_adaptor = $self->param('comparaDBA')->get_MetaContainer;
	    $tree_file =~ s/.*\/([^\/]+)$/$1/;
