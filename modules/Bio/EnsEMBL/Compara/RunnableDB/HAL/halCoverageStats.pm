...
sub run {
    my $self = shift;
    my $cmd_out = $self->get_command_output([$self->require_executable('halStats_exe'), '--coverage', $self->param('species_map')->{$self->param('genome_db_id')}, $self->param('hal_path')]);
    #parse the result to extract the columns 'Genome' and  'sitesCovered1Times'
    my @halCov = split /\n/, $cmd_out; #$self->param('temp_result');
    my %halCov_hash;
