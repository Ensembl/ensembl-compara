...
Bio::EnsEMBL::Compara::RunnableDB::ProteinTrees::HclusterRun
...
my $rdb = new Bio::EnsEMBL::Compara::RunnableDB::ProteinTrees::HclusterRun(
...
package Bio::EnsEMBL::Compara::RunnableDB::ProteinTrees::HclusterRun;
...
sub fetch_input {
    ...
    my $big_files_dir = $self->param('big_files_dir')
            || ( $self->param('use_temp') ? $self->worker_temp_directory : $self->param('cluster_dir') );
    $big_files_dir =~ s{/+$}{}; # FIXME: at the moment worker_temp_directory has a trailing '/' which has to be removed for consistency
    $self->param('big_files_dir', $big_files_dir);
}
...
sub run {
    ...
    if($self->input_job->retry_count <=10) {
        $self->gather_input();
        $self->run_hcluster();
    }
}
...
sub gather_input {      # FIXME: this subroutine should be moved into a separate analysis
  my $self = shift;

  my $starttime = time();

  my $cluster_dir = $self->param('cluster_dir') or die "'cluster_dir' is an obligatory parameter";
  my $big_files_dir = $self->param('big_files_dir');

  print "gathering input in $big_files_dir\n" if ($self->debug);

  my $cmd ="cat $cluster_dir/*.hcluster.cat > $big_files_dir/hcluster.cat";
  unless(system($cmd) == 0) {
    die "Error gathering category files for Hcluster by running cmd '$cmd' : $!";
  }
  printf("%1.3f secs to gather category entries\n", (time()-$starttime));
  $cmd ="cat $cluster_dir/*.hcluster.txt > $big_files_dir/hcluster.txt";
  unless(system($cmd) == 0) {
    die "Error gathering distance files for Hcluster by running cmd '$cmd' : $!";
  }
  printf("%1.3f secs to gather distance entries\n", (time()-$starttime));
}


sub run_hcluster {      # FIXME: this subroutine should also be moved into a separate analysis (SystemCmd is probably enough)
  my $self = shift;
  ...
  $cmd .= "-C $big_files_dir/hcluster.cat ";
  $cmd .= "-o $big_files_dir/hcluster.out ";
  $cmd .= " $big_files_dir/hcluster.txt";
...
#  my $mlss_adaptor          = $self->compara_dba->get_MethodLinkSpeciesSetAdaptor;
  ...
  if(defined($retry)) {
    ...
    $filename = "$big_files_dir/hcluster.out";

    if($big_files_dir ne $cluster_dir) {
