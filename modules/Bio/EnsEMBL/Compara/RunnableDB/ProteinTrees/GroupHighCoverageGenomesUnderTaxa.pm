...
Bio::EnsEMBL::Compara::RunnableDB::ProteinTrees::GroupHighCoverageGenomesUnderTaxa
...
package Bio::EnsEMBL::Compara::RunnableDB::ProteinTrees::GroupHighCoverageGenomesUnderTaxa;
...
sub fetch_input {
    ...
    foreach my $genome_db (@$genome_dbs) {
        ...
        if( ($coverage_depth eq 'high') or ($coverage_depth eq '6X')) {
            push @high_coverage_gdb_ids, $genome_db->dbID();
        }
    }
    my $high_coverage_gdb_id_string = join(',', @high_coverage_gdb_ids);
    ...
    foreach my $taxlevel (@$taxlevels) {
        push @species_sets, filter_genomes_by_taxlevel($dbc, $high_coverage_gdb_id_string, $taxlevel);
    }
...
sub filter_genomes_by_taxlevel {    # not a method
    my ($dbc, $high_coverage_gdb_id_string, $taxlevel) = @_;
    ...
    my $sql = qq{
        SELECT DISTINCT g.genome_db_id
         ...
         WHERE parent_name.name='$taxlevel'
           ...
           AND g.genome_db_id in ($high_coverage_gdb_id_string)
