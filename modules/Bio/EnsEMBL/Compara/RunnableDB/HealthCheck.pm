...
sub _run_compare_to_previous_db_test {
  ...
  if (defined $method_link_type && defined $current_genome_db_ids) {
		...
		if ($@ || !defined $previous_mlss) {
			my @local_names = map { $_->name() } @{$species_set};
			print "This pair of species (" .(join ",", @local_names) . ") with this method_link $method_link_type not do exist in this database $previous_db_url \n";
			return;
		}
