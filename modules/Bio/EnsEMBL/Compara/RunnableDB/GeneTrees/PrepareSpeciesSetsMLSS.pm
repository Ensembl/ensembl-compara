...
sub _write_ss {
    ...
    if ($self->param('reference_dba')) {
        ...
        if ((not $is_local_ss) and (not $ss) and not $self->param('master_create_missing')) {
...
sub _write_mlss {
    ...
    if ($self->param('reference_dba')) {
        ...
        $mlss or $self->param('master_create_missing') or die sprintf("The %s / %s MethodLinkSpeciesSet could not be found in the master database\n", $method->toString, $ss->toString);
    }
    unless ($mlss) {
