...
sub read_clusters_from_previous_db {
    ...
    my $sql_trees      = q{SELECT root_id, member_id FROM gene_tree_node JOIN gene_tree_root USING (root_id) WHERE clusterset_id = ? AND tree_type = "tree" AND member_id IS NOT NULL};
