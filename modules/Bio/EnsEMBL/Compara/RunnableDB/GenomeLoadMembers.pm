...
sub loadMembersFromCoreSlices
{
  ...
  SLICE: foreach my $slice (@slices) {
    ...
    foreach my $gene (@{$slice->get_all_Genes}) {
      ...
      if (lc($gene->biotype) eq 'protein_coding' || lc($gene->biotype) eq 'IG_V_gene' || lc($gene->biotype) eq 'IG_C_gene' || lc($gene->biotype) eq 'C_segment' || lc($gene->biotype) eq 'V_segment') {
        $self->{'realGeneCount'}++;
