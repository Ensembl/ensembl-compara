...
package Bio::EnsEMBL::Compara::RunnableDB::Pseudogenes::FilterInput::SearchPseudogenesInDataBase;
...
sub fetch_input
{
  ...
  $self->param('ref_dba', $self->get_cached_compara_dba('db_conn'));
  ...
  $self->param('tree_adaptor', $self->param('ref_dba')->get_GeneTreeAdaptor);
  $self->param('gene_member_adaptor', $self->param('ref_dba')->get_GeneMemberAdaptor);
}
...
sub run
{
  ...
  foreach my $line(@{$self->param('lines')})
  {
    ...
    foreach my $parent_gene(@$parents)
    {
      if((scalar @$genes) == 0)
      {
        store_info($self, $parent_gene, undef, undef, $input1, $input2, "NO GENE", $self->param('initial') + $execs);
        next;
      }
      ...
      foreach my $this_gene(@$genes)
      {
        ...
        {
	    unless($this_gene->slice->is_reference && $parent_gene->slice->is_reference)
            {
                store_info($self, $parent_gene, $this_gene, undef, $input1, $input2, "REFERENCE", $self->param('initial') + $execs);
                next;
            }
            ...
            store_info($self, $parent_gene, $this_gene, $aln_data, $input1, $input2, "ALIGNMENT", $self->param('initial') + $execs);
        } 
      }
    }
  }
...
sub store_info
{
  ...
  my $sql = "REPLACE INTO pseudogenes_data (parent_id, pseudogene_id, parent_transcript_id, transcript_id, tree_id, score, evalue, parent_species, parent_query, parent_type, pseudogene_species, pseudogene_query, pseudogene_type, status, filepath, line) VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)";
  my $sth = $self->compara_dba->dbc->prepare($sql);
  if($aln_data)
  {
    $sth->execute($parent->stable_id, $pseudogene->stable_id, $aln_data->{pseudogene_transcript}, $aln_data->{parent_transcript}, get_tree_id($self, $parent->stable_id), $aln_data->{score}, $aln_data->{evalue}, $self->param('ref_species'), $parent_input, $self->param('ref_type'), $self->param('target_species'), $pseudo_input, $self->param('target_type'), "OK", $self->param('path'), $line);
  }
  else
  {
    $sth->execute($parent->stable_id, $pseudogene ? $pseudogene->stable_id : undef, undef, undef, undef, undef, undef, $self->param('ref_species'), $parent_input, $self->param('ref_type'), $self->param('target_species'), $pseudo_input, $self->param('target_type'), $status, $self->param('path'), $line);
  }
  $sth->finish;
}
...
sub get_correct_adaptor
{
    ...
    if($type eq 'gene')
    {
      return $ref_dba->get_GeneMemberAdaptor;
    }
    elsif($type eq 'sequence')
    {
      return $ref_dba->get_SeqMemberAdaptor;
    }
...
sub compute_alignment
{
  ...
  $self->run_command($self->param('aligner')." -query $worker_tmp/query_sequence.fa -subject $worker_tmp/subject_sequence.fa -out $worker_tmp/output.txt -evalue 1e-4 -outfmt \'6 qlen slen length score evalue\'", { die_on_failure => 1 });
