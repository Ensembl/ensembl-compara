...
package EnsEMBL::Draw::GlyphSet_simpler;
...
sub _init {
  ...
  foreach my $f (@$features) {
    ...
    my $label = $no_labels ? undef : $f->{'feature_label'};
...
sub highlight {
  ...
  if ($self->{'config'}->get_option('opt_highlight_feature') != 0 and
      exists $f->{'display_name'} and
      grep $_ eq $f->{'display_name'}, $self->highlights) {
