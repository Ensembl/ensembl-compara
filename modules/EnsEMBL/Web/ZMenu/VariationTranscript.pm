...
sub content {
  ...
  my $trans_id   = $object->param('vt') || die 'No transcript stable ID value in params';
  my $exon_id    = $object->param('e')  || die 'No exon stable ID value in params';
  my $transcript = $object->database('core')->get_TranscriptAdaptor->fetch_by_stable_id($trans_id);
  my $protein_id = $transcript->translation ? $transcript->translation->stable_id : '';

  $self->caption($object->species_defs->AUTHORITY . ' Gene');
  
  $self->add_entry({
    label => $transcript->external_name eq '' ? $trans_id : $transcript->external_db . ': ' . $transcript->external_name
  });
  
  if ($object->type ne 'Transcript') {
    $self->add_entry({
      type       => 'Gene',
      label_html => $object->stable_id,
      link       => $object->_url({ type => 'Gene', action => 'Summary', g => $object->stable_id })
    });
  }
  ...
  $self->add_entry({
    type       => 'Transcript',
    label_html => $trans_id,
    link       => $object->_url({ type => 'Transcript', action => 'Summary', t => $trans_id })
  });
  ...
  if ($protein_id) {
    $self->add_entry({
      ...
      link       => $object->_url({ type => 'Transcript', action => 'ProteinSummary', t => $trans_id })
    });
