...
sub configure {
  ...
  if ($hub->object_types->{$hub->type}) {
    ...
    $hub->components = $module_name->new_for_components($hub, $self->builder, { tree => new EnsEMBL::Web::Tree }, $referer->{'ENSEMBL_ACTION'}, $referer->{'ENSEMBL_FUNCTION'}) if $self->dynamic_use($module_name);
  }
}
