...
sub content {
  ...
  my ($exonA, $exonB, $upstream, $exon_info, $intron_info);
  ...
  my $sscon_dot_length    = $seq_cols - 2*($sscon % ($seq_cols / 2)); # works out length needed to join intron ends with dots
  ...
  if ($flanking && !$only_exon) {
    ...
    $exon_info = { exint => "5' upstream sequence", Sequence => qq{<span class="exons_flank">$upstream</span>} };
    $table->add_row($exon_info);
  }
  
  # Loop over each exon
  for (my $j = 1; $j <= scalar @el; $j++) {
    $exonA = $el[$j-1];
    $exonB = $el[$j];
    ...
    $exonA_ID = "<strong>$exonA_ID</strong>" if $entry_exon && $entry_exon eq $exonA_ID;
  ...
  if ($flanking && !$only_exon) {
    ...
    $exon_info = { exint => "3' downstream sequence", Sequence => qq(<span class="exons_flank">$downstream</span>) };
