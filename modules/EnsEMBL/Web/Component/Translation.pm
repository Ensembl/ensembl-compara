...
# Get parameters to be passed to dasconfview script
    ...
    my $hidden_tmpl = qq(<INPUT type="hidden" name="%s" value="%s" />\n);
    my $hidden = '';
	...
	if (defined(my $v = $gene->param($param))) {
	    if (ref $v eq 'ARRAY') {
		foreach my $w (@$v) {
		    $hidden .= sprintf( $hidden_tmpl, $param, $w );
		    $params .= ";$param=$w";
		}
	    } else {
		$hidden .= sprintf( $hidden_tmpl, $param, $v );
		$params .= ";$param=$v";
	    }
	}
    }
...
# Manage Sources button
    my $conf_submit = qq(
<FORM name="dasConfigForm" method="post">
...
# Get the DAS configuration
    ...
    my $das_attribute_data = $das_collection->get_DASAdaptor_attributes_by_method( "name", "authority", "description", "conftype", "active", "type" );
...
# Build up the type of feature location	: see FLabels hash few lines above for location types
	...
	foreach my $feature( sort{ $a->das_type_id    cmp $b->das_type_id ||  $a->das_feature_id cmp $b->das_feature_id || $a->das_note cmp $b->das_note } @features ){

	    next if ($feature->das_type_id() =~ /^(contig|component|karyotype|INIT_MET)$/i || $feature->das_type_id() =~ /^(contig|component|karyotype|INIT_MET):/i);
...
<TABLE class="hidden">%s
  ...
  my $space_row = qq(
 <TR><TD colspan=4 height=5><IMG src="/img/blank.gif" height="5" /></TD></TR>);

	if( scalar( @rhs_rows ) == 0 ){
	    push( @rhs_rows, "No annotation" );
	}

	$shtml = sprintf( $table_tmpl, join("\n", @rhs_rows ) );

	$panel->add_row($label, $shtml );
    }
...
# Get the switched on sources
	...
	my $source_desc = $source->{description};
	my $selected = $selected_das{$name} ? 'CHECKED' : '';
	my $href = $source->{authority} || undef();
	my $label = $href ? qq(<A href="$href" target="new"> $name </A>) : $name;
