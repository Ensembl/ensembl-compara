package EnsEMBL::Web::Component::LRG::GeneSNPImage;
...
sub _content {
  my $self    = shift;
  my $no_snps = shift; 
  my $object_sl  = $self->object;

 #added for lrg BOF                                                                                                                                                                                              
  my $slice_orig = $object_sl->Obj;
  my $gene_s  =  shift @{$slice_orig->get_all_Genes}; #Bio::EnsEMBL::Gene
  my $stable_id = $object_sl->stable_id;
  my $input = new CGI;
  ...
  ## population of $object->__data->{'slices'}{'transcripts'} ??


  #print "transcript " . $transcript_slice->name."<BR>";
  #my $snps_transcript = $transcript_slice->get_all_VariationFeatures();
  #print Dumper($snps_transcript);
  #print "orig ". $slice_orig->name."<BR>";
  #print "subslice ". Dumper($sub_slices)."<BR>";
  # Fake SNPs -----------------------------------------------------------
  ...
  #my ($count_snps, $snps222222, $context_count) = $object->getVariationsOnSlice( $transcript_slice, $sub_slices  );  
...
#  print Dumper($object->__data->{'SNPS'});
## Intronless transcript top and bottom (to draw snps, ruler and exon backgrounds)
  foreach(qw(transcripts_top transcripts_bottom)) {
    ...
    $Configs->{$_}->{'snps'}        = $snps unless $no_snps;  #$object->__data->{'SNPS'}
