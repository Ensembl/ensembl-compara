...
sub content {
  ...
  foreach my $family_id (sort keys %$families) {
    ...
    $row->{'jalview'} = $self->jalview_link($family_id, 'Ensembl', $ensembl_members) . $self->jalview_link($family_id, '', \@all_pep_members) || 'No alignment has been produced for this family.';
...
sub jalview_link {
  ...
  my $count = @$refs;
  my $url   = $self->object->_url({ function => 'Alignments', family => $family });
  return qq(
