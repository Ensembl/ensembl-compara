  ...
  if (@urls) {
    ...
    foreach my $source (@urls) {
      if (ref($source) =~ /Record/) { ## from user account
        ...
        $table->add_row( { 'url'  => '<strong>'.$source->name.'</strong><br />'.$source->url.' ('.$source->species.')', 'date' => $self->pretty_date($date), 'save' => 'Saved', 'delete' => $link } );
      }
      else { ## temporary
        if ($user) {
        ...
        $table->add_row( { 'url'  => '<strong>'.$source->{'name'}.'</strong><br />'.$source->{'url'}.' ('.$source->{'species'}.')', 'date' => 'N/A', 'save' => $save, 'delete' => $detach } );
      }
    }
    $html .= $table->render;
  }
