...
sub content {
  ...
  my $url    = $self->ajax_url('ajax', 1) . ';r=' . $object->parent->{'params'}->{'r'} . ';code=' .  $object->param('code');
...
sub content_ajax {
  ...
  if ($upload->{'type'} eq 'upload') {
    ...
    if ($parser->nearest) {
      ...
      $html .= qq{<a href="/$upload->{'species'}/Location/View?r=} . $parser->nearest . '">' . $parser->nearest . '</a>';
    }
  }
