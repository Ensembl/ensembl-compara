...
sub content {
  ...
  my $adap = $hub->database($hub->param('db') || 'core')->get_adaptor('Marker');
 
  my $markers = $adap->fetch_all_by_synonym($hub->param('m'));
...
sub render_location {
  ...
  my @mfs            = $location->sorted_marker_features($m);
	my $c              = scalar @mfs;
	my $loc_text       = '<table>';
  ...
  my $priority       = 50;
	...
	if ($c) {
	    ...
	    my $start   = $mf->start;
	    my $end     = $mf->end;
	    my $url     = $hub->url({ type => 'Location', action => 'View', r => "$sr_name:$start-$end", m => $m_name }); 
