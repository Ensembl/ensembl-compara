...
sub content {
  ...
  my $wuc = $object->image_config_hash( "chromosome_$counter", 'MultiIdeogram', $ploc->species);
  $wuc->set_parameters({
  ...
  if (@{$other_locs}) {
    foreach my $loc ( @{$other_locs}) {
      $counter++;
      my $slice = $loc->{'slice'};
      my $wuc = $object->image_config_hash( "chromosome_$counter", 'MultiIdeogram',$loc->{'real_species'} );
      $wuc->set_parameters({
