...
sub build_sequence {
  ...
  my %class_to_style = (
    ...
    eo  =>  [ 8,  { 'background-color' => "#$styles->{'SEQ_EXONOTHER'}->{'default'}" } ],
    eg  =>  [ 9,  { 'color' => "#$styles->{'SEQ_EXONGENE'}->{'default'}", 'font-weight' => "bold" } ],
    c0  =>  [ 10, { 'background-color' => "#$styles->{'SEQ_CODONC0'}->{'default'}" } ],
    c1  =>  [ 11, { 'background-color' => "#$styles->{'SEQ_CODONC1'}->{'default'}" } ],
    cu  =>  [ 12, { 'background-color' => "#$styles->{'SEQ_CODONUTR'}->{'default'}" } ],
    sn  =>  [ 13, { 'background-color' => "#$styles->{'SEQ_SNP'}->{'default'}" } ],      
    si  =>  [ 14, { 'background-color' => "#$styles->{'SEQ_SNPINSERT'}->{'default'}" } ],
    sd  =>  [ 15, { 'background-color' => "#$styles->{'SEQ_SNPDELETE'}->{'default'}" } ],   
    snt =>  [ 16, { 'background-color' => "#$styles->{'SEQ_SNP_TR'}->{'default'}" } ],
    syn =>  [ 17, { 'background-color' => "#$styles->{'SEQ_SYN'}->{'default'}" } ],
    snu =>  [ 18, { 'background-color' => "#$styles->{'SEQ_SNP_TR_UTR'}->{'default'}" } ],
    siu =>  [ 19, { 'background-color' => "#$styles->{'SEQ_SNPINSERT_TR_UTR'}->{'default'}" } ],
    sdu =>  [ 20, { 'background-color' => "#$styles->{'SEQ_SNPDELETE_TR_UTR'}->{'default'}" } ],
    sf  =>  [ 21, { 'background-color' => "#$styles->{'SEQ_FRAMESHIFT'}->{'default'}" } ],
    aa  =>  [ 22, { 'color' => "#$styles->{'SEQ_AMINOACID'}->{'default'}" } ],
    var =>  [ 23, { 'color' => "#$styles->{'SEQ_MAIN_SNP'}->{'default'}" } ],
    end =>  [ 24, { 'color' => "#$styles->{'SEQ_REGION_CHANGE'}->{'default'}", 'background-color' => "#$styles->{'SEQ_REGION_CHANGE_BG'}->{'default'}" } ],
    bold => [ 25, { 'font-weight' => 'bold' } ]
  );
