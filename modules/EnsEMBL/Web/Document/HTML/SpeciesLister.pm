...
package EnsEMBL::Web::Document::HTML::SpeciesLister;
...
sub new
{
  ...
  my $self = {
      _image_type => '.png',
	  _setup_species_descriptions=>undef
  };
  bless $self, $class;
  return $self;
}
...
sub render {
  ...
  # give the possibility to add etxra info to $species_info via the function
  my $modify_species_info = $self->{_setup_species_descriptions};
  if(defined ($modify_species_info) && ref $modify_species_info eq "CODE"){
	$species_info = &$modify_species_info(\%$species_info);
  }
  ...
  my $full_list   = $self->render_species_list($species_defs, $user, $species_info, \%description, $fragment);
  ...
  ', $self->render_ajax_reorder_list($species_defs, $user, $species_info), $full_list);

  return $html;
}

sub render_species_list {
  my ($self,$species_defs, $user, $species_info, $description, $fragment) = @_;
  my (%check_faves, @ok_faves);
  
  foreach ($self->get_favourites($species_defs, $user, $species_info)) {
    push @ok_faves, $_ unless $check_faves{$_}++;
  }
  
  my $count    = @ok_faves;
  my $fav_html = $self->render_favourites($count, \@ok_faves, $description, $species_defs,$species_info);
  ...
  ', $fav_html, $self->render_species_dropdown($species_defs, $species_info, $description));
  
  return $html;
}

sub render_favourites {
  my ($self, $count, $ok_faves, $description, $species_defs,$species_info) = @_;
  ...
  if ($count > 3) {
    ...
    $html = sprintf('
      ...
      $self->render_with_images(\@first_half, $species_defs, $description,$species_info),
      $self->render_with_images($ok_faves,    $species_defs, $description,$species_info)
    );
  } else {
    $html = $self->render_with_images($ok_faves, $species_defs, $description,$species_info);
  }
...
sub render_species_dropdown {
  my ($self,$species_defs, $species_info, $description) = @_; 
...
sub render_ajax_reorder_list {
  my ($self, $species_defs, $user, $species_info) = @_;
  
  my @favourites = $self->get_favourites($species_defs, $user, $species_info);
...
sub render_with_images {
  my ($self, $species_list, $species_defs, $description, $species_info) = @_;
  ...
  foreach (@$species_list) {
    ...
    $html .= qq{
      <dt>
        <a href="/$_/Info/Index"><img src="/img/species/thumb_$_$self->{_image_type}" alt="$species_name" title="Browse $species_name" class="sp-thumb" height="40" width="40" /></a>
