...
package EnsEMBL::Web::Document::HTML::Configurator;
...
use base qw(EnsEMBL::Web::Document::HTML::Content);
...
sub init {  
  ...
  $page->local_context->tree($self->tree);
  $page->local_context->active($self->active);
  $page->local_context->caption($self->caption);
  $page->local_context->configuration(1);
}
