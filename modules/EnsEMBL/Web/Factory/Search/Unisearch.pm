...
sub createObjects { 
  ...
  warn "SEARCHING USING UNISEARCH TO FIND $idx in $species";
}
...
sub selectall_arrayref {
  ...
  return [] unless $db;
  my @all_res;
  my @kw = split /\s+/, join ' ', $self->param('q');
  warn join '-',@kw;
  foreach my $kw( @kw ) {
    my $sep = $kw =~ s/\*+$/%/ ? 'like' : '=';
    warn ">>>>> $species - $db - $kw - $sep <<<<";
    (my $t = $sql ) =~ s/\[\[KEY\]\]/$kw/g;
                 $t =~ s/\[\[COMP\]\]/$sep/g;
    warn $t;
    my $res = $db->db_handle->selectall_arrayref( $t );
    # $self->problem( 'Warning', 'SQL error'.$res, "<pre>$t</pre>" );
    push @all_res, @$res if $res;
  }
  return \@all_res;
}
...
sub search_GENE {
  ...
  push @databases, [ 'vega', 'Vega Loci', 'Vega Transcript', 'Vega Peptide' ] if $self->species_defs->other_species($species, 'databases')->{'DATABASE_VEGA'};
  push @databases, [ 'est',  'EST gene', 'EST Transcript', 'EST Peptide' ] if $self->species_defs->other_species($species, 'databases')->{'DATABASE_OTHERFEATURES'};
