package EnsEMBL::Web::Wizard::UserAccount;
...
sub validate {
  ...
  if ($result && $$result{'user_id'}) {  
    $parameter{'node'} = $is_ok; 
    $parameter{'set_cookie'} = $$result{'user_id'}; ## next node will set cookie
  }
  else {
...
http://$server/Multi/user_pw_reset?code=$code
