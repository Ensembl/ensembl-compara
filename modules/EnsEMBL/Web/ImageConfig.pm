...
sub add_tracks_gene {
  my $self = shift;
  my $branch = $self->create_submenu( 'genes', 'Genes' );
  my $dbs_hash = $sellf->species_defs->get_config('databases');
  foreach my $db_code ( keys %$dbs_hash ) ) {
    next unless exists $dbs_hash->{$db_code}{'tables'}{'gene'};
    ( my $db = $db_code ) =~ s/^ENSEMBL_//;
    $db = 'core' if $db eq 'db';
    my $h = $dbs_hash->{$db_code}{'table'}{'gene'}{'analyses'};
    foreach my $k (sort {$h->{$a} cmp $h->{$b} keys %$h ) {
      $branch->append( $self->create_node( $db_code.'_'.$k.'_gene', $h->{$k}.' genes', {
...
sub add_tracks_sub_type {
  my $self = shift;
  my $branch = $self->create_submenu( 'transcripts', 'Transcripts' );
  my $dbs_hash = $sellf->species_defs->get_config('databases');
  foreach my $db_code ( keys %$dbs_hash ) ) {
    next unless exists $dbs_hash->{$db_code}{'tables'}{'gene'};
    ( my $db = $db_code ) =~ s/^ENSEMBL_//;
    $db = 'core' if $db eq 'db';
    my $h = $dbs_hash->{$db_code}{'table'}{'gene'}{'analyses'};
    foreach my $k (sort {$h->{$a}{'name'} cmp $h->{$b}{'name'} keys %$h ) {
      my $v = $h->{$k};
      next unless $v->{'disp'};
      $branch->append( $self->create_node( $db.'_'.$k.'_transcript', $v->{'name'}.' transcripts', {
        'db'         => $db,
        'logic_name' => $k,
        'name'       => $v->{'name'},
