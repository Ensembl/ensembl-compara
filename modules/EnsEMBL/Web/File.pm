...
### /base_dir/timestamp/user_identifier/random_path/file_name.ext
...
###  - random_path is an optional quasi-random directory name, mainly needed for 
###    non-autogenerated files (which might have the same name)
...
### @return EnsEMBL::Web::File
  ...
  if ($file_path) {
    ...
    $self->{'random_path'}      = shift @path if scalar @path;
  }
  else {
    ## CREATING A NEW FILE
    if ($self->{'name'}) {
      ...
      $self->{'name'} =~ s/[^\w]/_/g;
      ## Include a random path in case we have multiple files with this name
      $self->{'random_path'} = random_string;
    }
    else {
