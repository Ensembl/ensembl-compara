...
#!/usr/bin/perl -w
package GO::Browser::HTMLBuilder;
...
sub draw_selected_term {
  ...
  my $HTMLString;
  my $rooturl="<a  href=go.cgi?accession=";
...
sub draw_term {
  ...
  my $rooturl="<a  href=http://www.fruitfly.org/~bradmars/cgi-bin/go.cgi?accession=";
  my $termm=GO::Tools::get_term($self->{dbh}, { acc => $acc });
...
sub _draw_tree {
  ...
  my @stack;
  my $vertical = {};
  my $termurl = "<a href=go.cgi?accession=";
  my $treeurl = "<a href=go.cgi?accession=";
  my $HTMLString ;

  foreach my $child (@{$tree->[0]}) {
    $HTMLString .= $termurl . $child->{acc} . $self->{urlpost} . ">" ;
	...
	if ($tree->[$horizontal]->{$stack[-1]}->[$vertical->{$stack[-1]}]->{acc}) {
	  ...
	  $HTMLString .= $termurl .$tree->[$horizontal]->{$stack[-1]}
	    ->[$vertical->{$stack[-1]}]->{acc} . $self->{urlpost} . ">" ;
...
sub draw_parents {
  ...
  foreach my $parent_a (@{$tree}) {
	...
	$HTMLString .= $rooturl . @{$parent_a}[$i]->{acc} . $self->{urlpost} . ">";
	$HTMLString .= '<img src="arrow2.jpg" border=0 hspace=1></a>';
      }
      $HTMLString .= "<spacer type=vertical length=4>";
      $HTMLString .= $rooturl . @{$parent_a}[-1]->{acc}. $self->{urlpost} . ">";
...
sub search {
  ...
  if (!$query->[0]->{name}) {confess;}
  foreach my $q (@{$query}) {
    $HTMLString .= "<a href=go.cgi?accession=$q->{acc}$self->{urlpost}>$q->{name}</a>";
    $HTMLString .= "<p>";
  }
  return $HTMLString; 
