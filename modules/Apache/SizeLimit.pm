...
BEGIN {
    ...
    } elsif ($Config{'osname'} eq 'dec_osf') {
	$HOW_BIG_IS_IT = \&dec_osf_size_check;
	...
	die $Config{'osname'}." Apache::SizeLimit not implemented on your platform.";
    }
}
...
sub exit_if_too_big {
	...
	($MAX_UNSHARED_SIZE && ($size - $share) > $MAX_UNSHARED_SIZE)) {
		...
		my $script = $r->parsed_uri->path;
		my $query = $r->parsed_uri->query;
		my $path_info = $r->parsed_uri->path_info;
		my $msg = "httpd process too big, exiting at SIZE=$size KB ";
		$msg .= " SHARE=$share KB " if ($share);
                $msg .= " REQUESTS=". ($REQUEST_COUNT - 1). " LIFETIME=$e seconds SCRIPT=$script QUERY=$query PATHINFO=$path_info";
		error_log($msg);

		if ($WIN32) {
		    CORE::exit(-2); # child_terminate() is disabled in win32 Apache
		} else {
			print STDERR "SO LONG FATTY\n";
		    $r->child_terminate();
		}
