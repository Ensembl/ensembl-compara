...
our @rel_spp = EnsEMBL::Web::Data::Species->find_all;
# create reverse hash so we can look up species ID by name
our %rev_hash;
foreach my $sp_obj (@rel_spp) {
  $rev_hash{$sp_obj->name} = $sp_obj->id;
}
...
foreach my $sp (@valid_spp) {
  ...
  my @stories = EnsEMBL::Web::Data::NewsItem->fetch_news_items($criteria, $attr);
  ...
  if ($total < 5) {
    ...
    $attr->{'limit'} = 5 - $total;
    my @generic =  EnsEMBL::Web::Data::NewsItem->fetch_news_items($criteria, $attr);
    if (@generic) {
      ## output generic stories
      $html .= qq(<ul class="spaced">\n);
      $html .= &output_stories(\@generic);
      $html .= qq(</ul>);
    } 
  }
