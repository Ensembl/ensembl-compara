...
# patch web_data type for protein features to 'domain' [default assumed by web code is feature)
# run this as follows to allow command prompts
# ( ./check_analysis_description-pf.pl > check_analysis_description.log_2 )
...
#list of domains
my @domains = qw(pfscan scanprosite superfamily pfam smart tigrfam pirsf prints);
...
foreach my $sp ( @species ) {
	...
	my $pfs = $dbh->selectall_arrayref(
	    'select distinct(a.logic_name)
               from protein_feature pf, analysis a
              where pf.analysis_id = a.analysis_id');
#	warn Dumper($dafs);
...
#	exit;
	...
	foreach my $id ( keys %$analyses ) {
	    ...
	    next ID unless (grep { $_->[0] eq $ln } @{$pfs});
	    if (grep {$ln eq lc($_)} @domains) {
		my $new_type_value = 'domain';
