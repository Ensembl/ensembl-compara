...
find "${PYTHON_SOURCE_LOCATIONS[@]}" -type f -name "*.py" | xargs pylint --rcfile pylintrc --verbose | tee "$PYLINT_OUTPUT_FILE"
grep -v "\-\-\-\-\-\-\-\-\-" "$PYLINT_OUTPUT_FILE" | grep -v "Your code has been rated" | grep -v "\n\n" | sed '/^$/d' > "$PYLINT_ERRORS"
! [ -s "$PYLINT_ERRORS" ]
rt1=$?
rm "$PYLINT_OUTPUT_FILE" "$PYLINT_ERRORS"

find "${PYTHON_SOURCE_LOCATIONS[@]}" -type f -name "*.py" | xargs mypy --config-file mypy.ini
rt2=$?
