...
process PREP_TASK_SHEET {
    ...
    input:
    tuple path(task_sheet), path(hal_cache)
    val(chrom_sizes)
    ...
    script:
    """
    ${params.prep_task_sheet_exe} $task_sheet ${hal_cache}/genome/chrom_sizes prepped_param_sets.tsv
    """
}
...
workflow {
    ...
    channel.value([params.input, hal_cache]) \
