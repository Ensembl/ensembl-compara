...
if __name__ == '__main__':
    ...
    query = f"""
        SELECT gdb1.name AS ref_species, gdb1.assembly AS ref_assembly, gdb2.name AS query_species, gdb2.assembly AS query_assembly,
        gm1.stable_id AS ref_gene_stable_id, gm1.display_label AS ref_gene_name, gm2.stable_id AS query_gene_stable_id, gm2.display_label AS query_gene_name,
        h.description AS homology_type, hm2.perc_id AS query_perc_id, hm2.perc_cov AS query_perc_cov
        FROM homology_member hm1 JOIN homology_member hm2 ON hm1.homology_id = hm2.homology_id
        JOIN homology h ON hm1.homology_id = h.homology_id
        JOIN {ref_db}.gene_member gm1 ON hm1.gene_member_id = gm1.gene_member_id
        JOIN gene_member gm2 ON hm2.gene_member_id = gm2.gene_member_id
        JOIN {ref_db}.genome_db gdb1 ON gm1.genome_db_id = gdb1.genome_db_id
        JOIN genome_db gdb2 ON gm2.genome_db_id = gdb2.genome_db_id
        JOIN {ref_db}.seq_member sm1 ON hm1.seq_member_id = sm1.seq_member_id
