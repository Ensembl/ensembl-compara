...
=item B<[--genome_dumps_dir dir]>

the path to the base directory of the genome dumps, as generated by the
DumpGenomes_conf pipeline.
...
Aliases are now allowed: 'soft' for 1, and 'hard' for 2
...
perl DumpMultiAlign.pl
  ...
  General configuration:
    ...
    [--dbname compara_db_name]
        the name of compara DB in the registry_configuration_file or any
        of its aliases. Uses "Multi" by default.
    [--genome_dumps_dir dir]
        the path to the base directory of the genome dumps, as generated by
        the DumpGenomes_conf pipeline.
...
$_->db_adaptor->dbc->disconnect_when_inactive($genome_dumps_dir) for @{$method_link_species_set->species_set->genome_dbs};
...
$masked_seq = 'soft' if $masked_seq eq '1';
$masked_seq = 'hard' if $masked_seq eq '2';
...
sub write_genomic_align_block {
  ...
  foreach my $this_genomic_align (@$genomic_aligns) {
    ...
    my $aligned_sequence;
    $this_genomic_align->original_sequence($this_genomic_align->get_sequence($masked_seq)) if $masked_seq;
    if ($original_seq) {
  ...
  Bio::EnsEMBL::Compara::RunnableDB::BaseRunnable->iterate_by_dbc($all_genomic_aligns,
    ...
    if (UNIVERSAL::isa($this_genomic_align, "Bio::EnsEMBL::Compara::GenomicAlignGroup")) {
      foreach my $this_sub_genomic_align (@{$this_genomic_align->get_all_GenomicAligns}) {
        ...
        $this_sub_genomic_align->original_sequence($this_sub_genomic_align->get_sequence($masked_seq)) if $masked_seq;
      }
    } else {
