...
public class BuildSynteny {
	...
	groupLinks(fset,maxDist,minSize);
	System.exit(0);
    }

    public static FeatureSetI groupLinks (FeatureSetI fset, int maxDist, int minSize) {
	...
	int[] featStart  = new int[fset.size()];
	int[] featHStart = new int[fset.size()];
	
	FeaturePair[] feat = new FeaturePair[fset.size()];
	FeaturePair[] hfeat = new FeaturePair[fset.size()];
	...
	for (int i = 0; i < fset.size(); i++) {
	    ...
	    feat[i] = sf;
	    hfeat[i] = sf;
	    featStart[i] = (int)sf.getLow();
	    featHStart[i] = (int)sf.getHLow();
	}
	
	
	QuickSort.sort(featStart,feat);
	QuickSort.sort(featHStart,hfeat);
	...
	for (int i= 0; i < feat.length; i++) {
		...
		//System.out.println("Dist is " + feat[i].getHname() + " " +  dist1 + " " + dist2 + " " + internum);
		
		// We've reached the end of a block
		if ((dist1 > maxDist) || (dist2 > maxDist) || internum > 5) {
	...
	if (size1 > 0.5*minSize && size2 > 0.5*minSize && feat.length > 0) {
	...
	for (int i=0; i < newfset.size(); i++) {
	    ...
	    //System.out.println("Processing feature " + fp.getHname() + " " + fp.getLow() + " " + fp.getHigh() + " - " + fp.getHLow() + " " + fp.getHHigh());
		...
		if (! prev.getHname().equals(fp.getHname()) || (Math.abs(dist1 - dist2) > (maxDist * 10)) || internum > 0 || ori == -1) {
