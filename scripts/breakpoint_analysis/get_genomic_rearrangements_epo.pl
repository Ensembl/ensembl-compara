...
=head1 DESCRIPTION
 ...
 This script finds the number of collinear genomic blocks for a pair of species in an epo msa. Information about the blocks are written to the given output file (this files tend to be 20mb and upwards in size).
 ...
 To run this script, you need to give it the following
    ...
    example run : perl get_genomic_rearrangements_epo.pl -species1 mus_caroli -species2 mus_pahari -kb 10 -output trial_b -reg_conf /nfs/users/nfs_w/wa2/Mouse_rearrangement_project/mouse_reg_livemirror.conf
