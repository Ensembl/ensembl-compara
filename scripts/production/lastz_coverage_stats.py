...
get_query_for_results = """
    ...
    AND
        tag IN ('reference_species', 'ref_genome_coverage', 'ref_genome_length', 'non_reference_species', 
        'non_ref_genome_coverage', 'non_ref_genome_length');
    """
...
def calculate_coverage_ratios(results):
    ...
    Returns:
        A nested dictionary with the structure {mlss_id: {mlss_tag: value}} and 
    ...
    for row in results:
        ...
        if mlss not in data:
            data[mlss] = {}
        try:
            data[mlss][tag] = int(value)
        except ValueError:
            data[mlss][tag] = value

    calculated_results = []
    for mlss, tags in data.items():
        # print(mlss,tags.get('reference_species'))
        ref_coverage_ratio = (
            tags.get("ref_genome_coverage") / tags.get("ref_genome_length")
        ) * 100
        non_ref_coverage_ratio = (
            tags.get("non_ref_genome_coverage") / tags.get("non_ref_genome_length")
        ) * 100
...
def process_database(url, output_file, release=None):
    ...
    with engine.connect() as connection:
        if release is None:
            ...
            ids = [str(row[0]) for row in initial_result]

            if ids:
                param_ids = {"mlss_ids": ids}
                result = connection.execute(text(get_query_for_results), param_ids)
                calculated_results = calculate_coverage_ratios(result)
            ...
            mlss_ids = [str(row[0]) for row in get_first_release_ids]
            ...
            mlss_ids.extend([str(row[0]) for row in get_rerun_mlsses])

            if mlss_ids:
                param_ids = {"mlss_ids": mlss_ids}
                result_query = connection.execute(
                    text(get_query_for_results), param_ids
                )
