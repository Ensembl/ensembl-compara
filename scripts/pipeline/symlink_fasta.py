...
if opts.cleanup_symlinks:
    for link in glob.glob(os.path.join(symlink_dir, '**/*.fasta'), recursive=True):
        if not os.path.exists(os.readlink(link)):
            print(f'Broken symlink: {0} to be removed'.format(link))
            os.remove(link)
...
if target_dir:
    for fasta_file in glob.glob(os.path.join(target_dir, '**/*.fasta'), recursive=True):
        ...
        if not os.path.exists(symlink_path):
            print(f'New symlink: {0} created for target: {1}'.format(symlink_path, fasta_file))
    ...
    if not os.path.exists(symlink_path):
        print(f'New symlink: {0} created for target: {1}'.format(symlink_path, target_file))
