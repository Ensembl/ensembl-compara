...
my $low_coverage_species_set = $species_set_adaptor->fetch_by_tag_value("name", "low-coverage");
if ($low_coverage_species_set) {
  ...
  foreach my $this_genome_db (@$all_genome_dbs) {
    ...
    next if ($this_genome_db->name eq "Ancestral sequences");
    next if ($this_genome_db->name eq "Caenorhabditis elegans");
