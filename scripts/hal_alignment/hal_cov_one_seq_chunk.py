        ...
        try:
            ...
            if match := declaration_line_re.fullmatch(line):
                ...
                if curr_seq_name in cov_stats:
                    raise ValueError(f"multiple occurrences of sequence '{curr_seq_name}' found") from exc
...
def main() -> None:
    ...
    parser.add_argument("ref_genome", help="Name of genome for which genomic coverage is calculated.")
    parser.add_argument(
        "--ref-sequence", metavar="STR", help="Name of sequence for which genomic coverage is calculated."
    )
    parser.add_argument(
        "--start", metavar="INT", required=True, type=int, help="Start of sequence chunk (0-based)."
    )
    parser.add_argument("--length", metavar="INT", required=True, type=int, help="Length of sequence chunk.")
    parser.add_argument(
        "--target-genomes", metavar="STR", required=True, help="Comma-separated list of target genomes."
    )
    ...
    if obs_num_positions != args.length:
        raise ValueError(f"sequence-length mismatch: {obs_num_positions} vs {args.length}")
