...
sub print_html_list {
  ...
  foreach my $this_method_link_species_set (@$these_method_link_species_sets) {
    my $string = $bottom_methods->{$this_method_link_species_set->method_link_type}->{string};
    if ($bottom_methods->{$this_method_link_species_set->method_link_type}->{bold}) {
      $string = "<b>$string</b>";
    }
    if ($bottom_methods->{$this_method_link_species_set->method_link_type}->{color}) {
      my $color = $bottom_methods->{$this_method_link_species_set->method_link_type}->{color};
      $string;# = "<font color=\"$color\">$string</font>";
    }
    print qq(<tr><th>$string</th><td class="bg4">);
    if ($use_names) {
      print "<b>", $this_method_link_species_set->name, "</b>: ";
    }
    print
